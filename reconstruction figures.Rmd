...
path<-"D:\\documents sd\\g drive\\dataset\\10-15 BSLC-training (1).csv"
dataset<-read.csv(path)

colndata<-colnames(dataset) %>% 
  str_remove("X") 

colnames(dataset)<-colndata

info<-dataset %>% 
  select(last_col(c(1:7)))

spectra<-dataset %>% 
  select(c(1:(ncol(dataset)-6))) 
...
```{r}
PCA <- princomp(spectra) #nfile=360, the last column c361 is the wtno
...
# #without center
# D<-denoise
d<-matrix(unlist(D),ncol=ncol(D)) %>% 
  baseline.modpolyfit(degree = 3, tol = 0.001, rep = 100)
d1<-d$corrected
colnames(d1)<- wtno

pmap<-cbind(info,(d1[,which((wtno>=751)&(wtno<=752))]),#nuclear acid
            (d1[,which((wtno>=1584)&(wtno<=1585))]),
            (d1[,which((wtno>=1003)&(wtno<=1004))]),#protein
            (d1[,which((wtno>=1451)&(wtno<=1452))]))%>% 
...
meanspec<-spectra %>% 
  cbind(info)%>% 
  gather("wtno","intensity",-c(colnames(info))) %>%
  group_by(group,wtno) %>% 
...
write.csv(meanspec,"E:\\R processed data\\12-25 origin mean spectra.csv")

hist<- info %>% 
  cbind((spectra[,which((wtno>=749)&(wtno<=750))]),#nuclear acid
            (spectra[,which((wtno>=919)&(wtno<=920))]),
            (spectra[,which((wtno>=1003)&(wtno<=1004))]),#protein
            (spectra[,which((wtno>=1451)&(wtno<=1452))])) %>% 
  set_names(c(colnames(info),"p1","p2","p3","p4")) %>% filter(group!="buffer") %>% 
